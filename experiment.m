...
for scr=1:1   % Changed to only lcd due to manual testing of kindle - DM
   ...
   for trl=1:3 % Number of Trials
       for lns=1:3 %Number of Samples
           dialogwait('Trial '+trial(trl)+ ' | ' + 'Insert lens ' +lens(lns));
           for isLns=1:2 %Lens Position (up or down)
                   dialogwait('Trial '+trial(trl)+ ' | '+ 'Make sure lens is '+isLens(isLns));
               for prd=1:2 %Number of period variations in the images to test
                   for ornt=1:2 %number of orientations in the images to test
                      imshow(strcat(num2str(period(prd)),'px',orientation(ornt),'.png'),'InitialMagnification',100);  

                         % If unusual zoom things start happening, remove the last two imshow() arguments
                        movegui(H,'center');
